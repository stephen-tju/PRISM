...
# Free combination, each combination will be a curve line
...
used_case_ids = set(df["case_id"].tolist())
copy_commands = [f"cp -r ./cases/case_{case_id} ./archives/cases_13B" for case_id in sorted(used_case_ids)]
with open("copy_cases.sh", "w") as f:
    f.write("\n".join(copy_commands) + "\n")
...
fig, axs = plt.subplots(1, 5, figsize=(16, 3), dpi=400)
...
for curve_idx, curve in enumerate(curve_list):
    ...
    for i, avg_df in grouped_df:
        ...
        # print(f"Curve: {curve_label}, Metric: {plot_metrics[i]}-{plot_quantile[i]}")
        # for qps_val in avg_df["qps"]:
        #     case_ids_at_qps = df_curve[df_curve["qps"] == qps_val]["case_id"].tolist()
        #     print(f"  QPS {qps_val}: {case_ids_at_qps}")

        ax.plot(avg_df["qps"], avg_df["metric_value"], color=color, linewidth=1.5, linestyle='-', label=curve_label)
        ax.set_title(f"{plot_metrics[i]} - {plot_quantile[i]}", fontsize=8)
