...
fig, axs = plt.subplots(w, h, figsize=(30, 30), dpi=400)
...
for curve_idx, curve in enumerate(curve_list):
    ...
    for i, metric in enumerate(plot_metrics):
        for j, quantile in enumerate(plot_quantile):
            ...
            curve_x, curve_y = [], []  # curve data

            for k in range(len(df_curve)):
                qps_val = df_curve["qps"].iloc[k]
                if quantile not in df_curve["metrics_info"].iloc[k][metric]:
                    continue
                data_point = df_curve["metrics_info"].iloc[k][metric][quantile]
                case_id = df_curve["case_id"].iloc[k]

                if not np.isfinite(data_point): 
                    print(f"⚠️ Skipping invalid data: case_id={case_id}, qps={qps_val}, metric={metric}, quantile={quantile}, value={data_point}")
                    continue 

                ax.scatter(qps_val, data_point, color=color, marker='o', s=2, label=curve_label if k == 0 else "")
                # ylim
                if ylim is None:
                    ax.text(qps_val, data_point, f"{case_id}", fontsize=4, ha='right')
                else:
                    if data_point >= ylim[0] and data_point <= ylim[1]:
                        ax.text(qps_val, data_point, f"{case_id}", fontsize=4, ha='right')

            ax.set_title(f"{metric}-{quantile}", fontsize=8)
