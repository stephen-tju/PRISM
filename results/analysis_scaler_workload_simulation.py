...
def process_case_wkpd(case_id):
    ...
    # 3. Simulation based on npred
    cur_num_inst = 4
    record_num_inst = []
    workload_per_instance = 21000.0 * 2
    lock = 0
    for pred in npred:
        if lock == 0:
            num_inst = pred / workload_per_instance
            num_inst_up = np.ceil(num_inst + 0.2)
            num_inst_down = np.ceil(num_inst + 0.3)
            
            if num_inst_up > cur_num_inst:
                cur_num_inst = num_inst_up
                lock = 3
            elif num_inst_down < cur_num_inst:
                cur_num_inst = num_inst_down
                lock = 3
        else:
            lock -= 1
                
        record_num_inst.append(cur_num_inst * workload_per_instance)

    # # 4. Covering num inst 
    # # Ceil
    # covering_num_inst = np.ceil(np.array(npred) / workload_per_instance) * workload_per_instance
    ...
    plt.plot(record_num_inst, label="Current Num Instances")
    # plt.plot(covering_num_inst, label="Covering Num Instances", linestyle='--')
    # plt.step(relative_timestamps, discr_max_workloads_per_interval, 
        # label="Optimal Num Instances", where='post')
    plt.legend()

    plt.savefig(f"{case_dir}/scaler_info_simul.png")

if __name__ == '__main__':
    process_case_wkpd(274)
