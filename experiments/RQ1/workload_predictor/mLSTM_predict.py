...
class workload_predictor:
    def __init__(self, train_data, sequence_length=25, hidden_dim=64, num_epochs=50, batch_size=32, lr=0.001, seed=42):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 10 == 0:
                logger.info("Epoch %d/%d, Loss: %f", epoch + 1, num_epochs, train_loss / len(self.train_loader))
...
def convert_to_ds(timestamp):
    dt = datetime.fromtimestamp(int(timestamp))
    return dt.strftime('%Y-%m-%d %H:%M:%S')

def preprocess_data(df, data_type):
    if data_type == 'prompt':
        data = df[['TimeWindow', 'Request tokens']]
    elif data_type == 'response':
        data = df[['TimeWindow', 'Response tokens']]
    else:
        raise ValueError("data_type must be either 'prompt' or 'response'")

    data['TimeWindow'] = data['TimeWindow'].apply(convert_to_ds)
    data.columns = ['ds', 'y']
    return data
...
def train_and_predict(data):
    ...
    predictor = workload_predictor(train_data)
    mean_test_y = test_data.mean()
    predictions = []
    for ground_truth in test_data:
        next_prediction = predictor.predict_next_window()
        predictions.append(next_prediction)
        predictor.add_history(ground_truth)
    return test_data, predictions


def process_dataset(dataset_name):
    dataset_path = f"../../../data/workloads/Azure/Azure_{dataset_name}_cleaned.csv"
    ...
    for data_type in ['prompt', 'response']:
        ...
        test_data, prediction = train_and_predict(processed_data)
        error_percentage, max_error = mean_absolute_percentage_error(test_data, prediction)
        logger.info(f"{data_type}s: Mean APE {error_percentage:.3f} | Max APE {max_error:.3f}")
